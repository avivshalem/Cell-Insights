...
class MultiCellTracker:
    ...
    def propagate_tracklets(self, time):
        ...
        image = np.asarray(Image.open((self.config.get_image_file(time))))
...
class CellTrack:
    ...
    def add_time_step(self, mask_id, mask_indices):
        ...
        """
        len_mask = len(mask_indices[0])
        time, _ = mask_id
        self.masks[time] = mask_indices
        self.last_assigned_obj = mask_id

        # Aviv
        self.center_of_mass[time] = np.sum(mask_indices[0])/len_mask, np.sum(mask_indices[1])/len_mask
...
class TrackingConfig:
    ...
    def get_segmentation_masks(self, time_step):
        segmentation = np.asarray(Image.open((self.segm_files[time_step])))
        segmentation = np.squeeze(segmentation)
        return segmentation, get_indices_pandas(segmentation, background_id=0)
