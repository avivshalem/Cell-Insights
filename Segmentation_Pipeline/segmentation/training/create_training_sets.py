...
def generate_data(img, mask, search_radius, max_mal, cell_type, crop_size, mode, min_area,
                  scale, path_train_sets, i, slice_idx=None):
    ...
    for y in range(ny):
        for x in range(nx):
            ...
            img_crop, mask_crop, cell_dist_crop, neighbor_dist_crop  = get_crop(x, y, crop_size,
                                                                                            img, mask, cell_dist,
                                                                                            neighbor_dist)
...
def get_gt_settings(gt_id_list):
    ...
    for gt_id in gt_id_list:
        seg_gt = imageio.imread(str(gt_id))
...
def create_ctc_training_sets(path_data, path_train_sets, cell_types):
    ...
    for cell_type in cell_types:
        ...
        for mode in modes:
            ...
            if len(list((path_train_sets / "{}_{}".format(cell_type, mode) / 'train').glob('*.png'))) > 0:
                continue
            ...
            if mode == 'GT':  # Check with TRA-GT if image is fully annotated
                ...
                seg_gt_ids = sorted((path_data / 'training_datasets' / cell_type / '01_GT' / 'SEG').glob('*.png'))
                seg_ids = sorted((path_data / 'training_datasets' / cell_type / '01').glob('*.png'))
                ...
                for i in range(len(seg_gt_ids)):

                    # seg_gt = imageio.imread(str(seg_gt_ids[i]), as_gray=True).astype(int)
                    seg_gt = np.asarray(Image.open(str(seg_gt_ids[i])).convert('L'))


                    # img = imageio.imread(str(seg_ids[i]), as_gray=True)
                    img = np.asarray(Image.open(str(seg_ids[i])).convert('L'))


                    img, seg_gt = foi_correction_train(cell_type, img, seg_gt)

                    # if scale != 1:
                    #     img, seg_gt = downscale(img=img, seg_gt=seg_gt, scale=scale)

                    # min-max normalize image to 0 - 65535
                    img = 65535 * (img.astype(np.float32) - img.min()) / (img.max() - img.min())
                    img = np.clip(img, 0, 65535).astype(np.uint16)

                    generate_data(img=img, mask=seg_gt, search_radius=search_radius,
                                      max_mal=max_mal, cell_type=cell_type, crop_size=crop_size, mode=mode,
                                      min_area=min_area, scale=scale,
                                      path_train_sets=path_train_sets, i=i)
                ...
                if not train_val_ids:  # no split available
                    ...
                    with open(Path.cwd() / "segmentation" / "training" / "splits" / '_ids_{}_{}.json'.format(cell_type, mode),
