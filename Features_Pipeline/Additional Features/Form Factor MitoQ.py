...
CounterXL = 0
df1 = pd.read_excel(r'\output_MitoQ_Severe.xlsx')
df2 = pd.read_excel(r'\output_MitoQ_Risk.xlsx')
df3 = pd.read_excel(r'\output_MitoQ.xlsx')
df = pd.concat([df1, df2, df3])
df_new = pd.DataFrame()
savePlots = r'\Form_Factor_MitoQ'
os.makedirs(savePlots, exist_ok=True)
cols = df.iloc[:, 0]
new_cols = []
for itemIndex, item in enumerate(cols):
    if 'HCMedia' in item or 'HDMedia' in item:
        continue
...
for col in range(len(new_cols)):
    if 'HCMedia' in new_cols[col]:
        ...
        type_cols.append('HD')
    elif 'MitoQ' in new_cols[col]:
        type_cols.append('MitoQ 1µM')
...
for item in df.columns:
    ...
    # Display the R-squared value on the plot
    # ax2.text(0.05, 0.9, f'R-squared = {r2:.2f}', transform=ax2.transAxes, fontsize=12)
    ax2.set_title(f"{item}")
    fig.savefig(savePlots + fr'\{item}_Box_MitoQ.png', bbox_inches='tight', dpi=900)
