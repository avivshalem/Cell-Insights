...
def inference_2d_ctc(model, data_path, result_path, device, batchsize, args, num_gpus=None):
    ...
    for sample in dataloader:
        ...
        if batchsize > 1:  # all images in a batch have same dimensions and pads
            print('Batch size larger than 1?')
        ...
        for h in range(len(prediction_border_batch)):
            ...
            if model_settings['label_type'] == 'distance':
                ...
                tiff.imsave(str(result_path / ('mask' + file_id)), prediction_instance)
                if save_raw_pred:
                    tiff.imsave(str(result_path / ('cell' + file_id)), prediction_cell_batch[h, ..., 0].astype(np.float32))
                    tiff.imsave(str(result_path / ('raw_border' + file_id)), prediction_border_batch[h, ..., 0].astype(np.float32))
                    tiff.imsave(str(result_path / ('border' + file_id)), border.astype(np.float32))
    ...
    if args.artifact_correction:
        ...
        for prediction_instance_id in prediction_instance_ids:
            ...
            tiff.imsave(str(prediction_instance_id), prediction_instance.astype(np.uint16))
...
def inference_3d_ctc(model, data_path, result_path, device, batchsize, args, num_gpus=None):
    ...
    for sample in dataloader:
        ...
        prediction_cell_batch = np.zeros(shape=img_batch.shape, dtype=np.float32)
        prediction_border_batch = np.zeros(shape=img_batch.shape, dtype=np.float32)

        img_batch = img_batch.to(device)

        # Go through z-dimension and process different frames in parallel
        for z in range(img_batch.shape[1]):

            img_slice_batch = img_batch[:, z, ...]
            img_slice_batch = img_slice_batch[:, None, ...]

            prediction_slice_border_batch, prediction_slice_cell_batch = net(img_slice_batch)
            prediction_cell_batch[:, z, ...] = prediction_slice_cell_batch[:, 0, ...].cpu().numpy()
            prediction_border_batch[:, z, ...] = prediction_slice_border_batch[:, 0, ...].cpu().numpy()

        # Get rid of pads
        prediction_cell_batch = prediction_cell_batch[:, :, pad_batch[1]:, pad_batch[0]:]
        prediction_border_batch = prediction_border_batch[:, :, pad_batch[1]:, pad_batch[0]:]
        ...
        for h in range(len(prediction_border_batch)):
            ...
            if model_settings['label_type'] == 'distance':
                ...
                if save_raw_pred:
                    tiff.imsave(str(result_path / ('cell' + file_id)), prediction_cell_batch[h].astype(np.float32))
                    tiff.imsave(str(result_path / ('raw_border' + file_id)), prediction_border_batch[h].astype(np.float32))
                    tiff.imsave(str(result_path / ('border' + file_id)), border.astype(np.float32))
