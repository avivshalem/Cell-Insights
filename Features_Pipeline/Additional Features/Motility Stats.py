...
def extract_pValue(df, labels):
    p_value_dict = {}
    for i, label1 in enumerate(labels):
        for j, label2 in enumerate(labels):
            if i < j:
                group1 = df.loc[df['Group'] == label1, 'Normalized Feature']
                group2 = df.loc[df['Group'] == label2, 'Normalized Feature']

                # Test for normality using the Shapiro-Wilk test
                _, p_value1 = stats.shapiro(group1)
                _, p_value2 = stats.shapiro(group2)

                is_normal1 = p_value1 > 0.05  # You may adjust the threshold
                is_normal2 = p_value2 > 0.05  # You may adjust the threshold
                ...
                p_value_dict[f"{label1}_vs_{label2}"] = p_value
...
bins = [-1, 0, 90, 112, 998, 1000]  # Bins are inclusive of right value by default
labels = ['HC', 'Premanifest', 'Mild', 'Severe', 'HGPS']
...
data_to_plot = [df.loc[df['Group'] == label, 'Normalized Feature'] for label in labels]


p_value_dict = extract_pValue(df, labels)
...
bp2 = ax3.boxplot(data_to_plot, patch_artist=True, medianprops=medianprops, whis=5)
...
median_group_values = [np.median(df.loc[df['Group'] == label, 'Normalized Feature']) for label in labels]
...
ax3.set_ylim([-15, 15])
savePlots = r'C:\CellInsights\Motility'
...
for i, label1 in enumerate(labels):
    for j, label2 in enumerate(labels):
        ...
        p_value = p_value_dict.get(pair_key, 1)
        star_annotation = 'N.P.'
        if p_value < 0.001:
            star_annotation = '***'
        elif p_value < 0.01:
        ...
        if star_annotation:
            x_line_center = (i + 1 + j + 1) / 2  # Adding 1 because the index starts from 1 in the plot
            y_line_center = current_max_y + offset

            # Draw the line
            ax3.plot([i + 1, j + 1], [y_line_center, y_line_center], color='grey', linestyle='--')

            # Annotate text
            ax3.annotate(star_annotation, xy=(x_line_center, y_line_center),
                         xytext=(0, 1.5), textcoords='offset points', ha='center', fontsize=6)
