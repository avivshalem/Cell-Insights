%%% Correlations
%%%
load('\NatureAviv\Compunds2\MitoQ_Media_DMSO_Cleaned_75\xys_TRJs_MitoQ.mat')
load('\NatureAviv\Compunds2\MitoQ_Media_DMSO_Cleaned_75\xys_full_MitoQ.mat')
load('\NatureAviv\Compunds2\MitoQ_Media_DMSO_Cleaned_75\DataFileMitoQ.mat')
...
AllCountHDAll = find(data.CellType == uniques(2,:));
AllCountMitoQAll = find(data.CellType == uniques(3,:));
AllCountHCMediaAll = find(data.CellType == uniques(4,:));
AllCountHDMediaAll = find(data.CellType == uniques(5,:));
%
% combinedIndex = combinedIndexCached;
% Correlations Loop Fixed X AXIS
folderCurrent = "Results\MitoQ";
mkdir(folderCurrent);
...
for i=1:length(combinedIndex)
    ...
    if data.CAP(i) == 0 && data.MSD10(i) < 10
        combinedIndex(i) = 0;
    end
    if data.CAP(i) > 20 && data.CAP(i) < 90 && data.MSD10(i) > 7000
        combinedIndex(i) = 0;
    end
    if data.CAP(i) > 90 && data.CAP(i) < 114 && data.MSD10(i) > 5000
        combinedIndex(i) = 0;
    end
    if data.CAP(i) >= 114 && data.MSD10(i) > 4500
        combinedIndex(i) = 0;
    end
%     if (round(data.CAP(i)) == 100 || round(data.CAP(i)) == 104 || round(data.CAP(i)) == 106 ...
%             || round(data.CAP(i)) == 110 || round(data.CAP(i)) == 111 ...
%             || round(data.CAP(i)) == 114 || round(data.CAP(i)) == 115 ...
%             || round(data.CAP(i)) == 126 || round(data.CAP(i)) == 129 || round(data.CAP(i)) == 141) ...
%             && data.MSD10(i) > 6500
%         combinedIndex(i) = 0;
%     end
end
...
F    = getframe(fig);
exportgraphics(t,strcat(folderCurrent, '\_z_CAP_Correlations_Media_', num2str(iBig), '.png'),'Resolution',500)
end
%% Both
close all
tileFirst = 1 + floor(size(feature_vectors,2) / 2);
...
F    = getframe(fig);
exportgraphics(t,strcat(folderCurrent, '\_z_CAP_Correlations_Both_', num2str(iBig), '.png'),'Resolution',500)
end
%%
% % close all
% %
...
a3Pol = xys(temp_cellType == "'MitoQ'");
% a4Pol = xys(temp_cellType == "HCMedia'");
...
legend('HC','HD' ,'MitoQ 1µM', 'Location','bestoutside');
    axis square;
    box on
g = gcf;
...
a1Pol = xys(temp_cellType == "'HC'");
a2Pol = xys((CAPS>20) & (CAPS<90)); %risk
a3Pol = xys((CAPS>90) & (CAPS<115)); %mild
a4Pol = xys((CAPS>115) & (CAPS<999)); %severe
a5Pol = xys((CAPS>0) & (CAPS<9)); %mq risk
a6Pol = xys((CAPS>9) & (CAPS<11.5)); %mq mild
a7Pol = xys((CAPS>11.5) & (CAPS<20)); %mq severe
...
hold on;
param.markertype='.-'; 
get_MSD(a6Pol, 10, param);
hold on;
param.markertype='.-'; 
get_MSD(a7Pol, 10, param);
hold on;
...
% legend('HC','HD' ,'MitoQ', 'HCMedia', 'HDMedia', 'Location','bestoutside');
legend('HC','HD - Premanifest','HD - Mild','HD - Severe', ...
    'MitoQ - Premanifest','MitoQ - Mild','MitoQ - Severe', ...
...
saveas(g, strcat(folderCurrent, '\z_MSD_2.png'))
%%
%%% MSD vs TLAG PATIENT
% close all force
figure
...
temp_Patient = data.CAP(combinedIndex);
...
half_indices = perm_indices(1:round(n / 20));
...
ylabel('Occurrence');
    ...
    box on
        ...
        g = gcf;
g.WindowState = 'maximized';
saveas(g, strcat(folderCurrent, '\z_Theta_PDF_HD.png'))
% hold on;
...
ylabel('Occurrence');
    ...
    box on
        ...
        g = gcf;
g.WindowState = 'maximized';
saveas(g, strcat(folderCurrent, '\z_Theta_PDF_MitoQ.png'))
...
hold on;
param.markertype=''; % HD
get_ACF1(a6Pol, 10, param);
hold on;
param.markertype=''; % HD
get_ACF1(a7Pol, 10, param);
hold on;
% colororder(newcolors)
...
g = gcf;
g.WindowState = 'maximized';
saveas(g, strcat(folderCurrent, '\z_ACF_2.png'))
%%
%%% ACF vs TLAG PATIENT
close all force
param.dim=2; 
...
hold on;
param.markertype=':'; % HD
get_dR_PDF(a6Pol, 10, param);
hold on;
param.markertype=':'; % HD
get_dR_PDF(a7Pol, 10, param);
hold on;
...
g = gcf;
g.WindowState = 'maximized';
saveas(g, strcat(folderCurrent, '\z_PDF_2.png'))
%%
%%% dR PDF PATIENT
close all force
param.dxmax=50;
...
for SMR=1:5
    ...
    elseif SMR == 2
        cells = a3Pol;
        titleSMR = {"Trajectories", "MitoQ 1µM"};
        color = newcolors{3};
        if size(cells,1) < numOfRandoms
            numOfRandomsIn = size(cells,1);
        else
...
%     elseif SMR == 3
%         cells = a4Pol;
%         titleSMR = strcat("Premanifest Trajectories");
%         color = newcolors{2};
%         if size(cells,1) < numOfRandoms
%             numOfRandomsIn = size(cells,1);
%         else
...
% Show Sunplot of Cells
    ...
    elseif SMR == 2
        cells = a3Pol;
        titleSMR = strcat("MitoQ Sunplot Trajectories");
        color = newcolors{3};
        if size(cells,1) < numOfRandoms
            numOfRandomsIn = size(cells,1);
        else
...
%     title(titleSMR);
    ...
    elseif SMR == 2
        cells = a3Pol;
        titleSMR = strcat("MitoQ Merged Trajectory");
        color = newcolors{3};
        if size(cells,1) < numOfRandoms
            numOfRandomsIn = size(cells,1);
        else
...
a3Pol_Activity = one_dim_xys(temp_cellType == "'MitoQ'",:);

classes = {a1Pol_Activity, a2Pol_Activity, a3Pol_Activity};
classNames = {'HC', 'HD', 'MitoQ 1µM'};
for i = 1:length(classes)
    aPol = classes{i};
    currNums = [];
        figure()
...
a3Pol_Activity = binarized(temp_cellType == "'MitoQ'",:);
...
uniquePatientsWell(contains(string(uniquePatientsWell),'MitoQ')) = 'MitoQ';
...
idx(uniquePatientsWell == 'MitoQ') = 2;
idx(uniquePatientsWell == 'HD') = 3;
...
legend('HC','MitoQ 1µM','HD', 'location', 'southwest');
...
% % hold on
% % plot(C(:,1),C(:,2),'kx','MarkerSize',15,'LineWidth',3);
% legend('HC','MitoQ','HD','LatBHigh');
% title('TSNE Clustering');
%% 3d look at features
close all

clc
num_dims = 3; % Reduce the data to 3 dimensions
uniquePatientsWell = unique(data.CAPPatient2(combinedIndex));
CAPS = str2double(regexp(string(uniquePatientsWell),'\d*','Match', 'once'));
risk = (CAPS>15) & (CAPS<90); %risk
mild = (CAPS>90) & (CAPS<115); %mild
severe = (CAPS>=115) & (CAPS<999); %severe

uniquePatientsWell(logical(contains(string(uniquePatientsWell),"'HDMedia")) & risk) = 'Premanifest - HDMedia';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),"'HDMedia")) & mild) = 'Mild - HDMedia';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),"'HDMedia")) & severe) = 'Severe - HDMedia';
uniquePatientsWell(contains(string(uniquePatientsWell),"0'HCMedia")) = 'HCMedia';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),'MitoQ'))  & risk) = 'Premanifest - MitoQ';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),'MitoQ'))  & mild) = 'Mild - MitoQ';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),'MitoQ'))  & severe) = 'Severe - MitoQ';
uniquePatientsWell(contains(string(uniquePatientsWell),"0'HC")) = 'HC';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),"'HD - ")) & risk) = 'Premanifest - HD';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),"'HD - ")) & mild) = 'Mild - HD';
uniquePatientsWell(logical(contains(string(uniquePatientsWell),"'HD - ")) & severe) = 'Severe - HD';

idx = double.empty(length(uniquePatientsWell),0);
idx(uniquePatientsWell == 'HC') = 1;
idx(uniquePatientsWell == 'Premanifest - MitoQ') = 2;
idx(uniquePatientsWell == 'Mild - MitoQ') = 3;
idx(uniquePatientsWell == 'Severe - MitoQ') = 4;
idx(uniquePatientsWell == 'Premanifest - HD') = 5;
idx(uniquePatientsWell == 'Mild - HD') = 6;
idx(uniquePatientsWell == 'Severe - HD') = 7;
scatterMap = [0 0 1; % Blue
             0 1 0; % Green
             0 0.8 0; % Darker green
             0 0.6 0; % Darkest green
             1 0 0; % Red
             0.8 0 0; % Darker red
             0.6 0 0;]; % Darkest red
...
% idx(uniquePatientsWell == 'HC') = 1;
% idx(uniquePatientsWell == 'Premanifest - MitoQ') = 2;
% idx(uniquePatientsWell == 'Mild - MitoQ') = 2;
% idx(uniquePatientsWell == 'Severe - MitoQ') = 2;
% idx(uniquePatientsWell == 'Premanifest - HD') = 3;
% idx(uniquePatientsWell == 'Mild - HD') = 3;
% idx(uniquePatientsWell == 'Severe - HD') = 3;
% scatterMap = [0 0 1; % Blue
%              0 1 0; % Red
%              1 0 0]; % Green
...
figure;
gscatter(Y(:,1), Y(:,2),idx, scatterMap);

% hold on
% plot(C(:,1),C(:,2),'kx','MarkerSize',15,'LineWidth',3);
% legend('HC','MitoQ','HD');
legend('HC',...
    'Premanifest - MitoQ','Mild - MitoQ', 'Severe - MitoQ',...
    'Premanifest - HD','Mild - HD', 'Severe - HD');
title('PCA Clustering');


Y = idx;
mappedX = tsne(zWell, 'NumDimensions', num_dims);
mappedX = Z;
% Perform k-means clustering
numClusters = 3; % Choose the number of clusters
[idx, C] = kmeans(mappedX, numClusters);

% gm = fitgmdist(mappedX, numClusters);
% idx = cluster(gm, mappedX);

% Create a scatter plot matrix of the t-SNE dimensions with colors assigned by cluster
% figure;
% scatter3(mappedX(:,1), mappedX(:,2), mappedX(:,3), 50, idx, 'filled', 'MarkerFaceAlpha', 0.8);
...
% colorbar;
% 
% % Add label text annotations to the 3D scatter plot
% hold on;
% for i = 1:size(zWell, 1)
%     text(mappedX(i,1), mappedX(i,2), mappedX(i,3), num2str(Y(i)), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'FontSize', 14, 'FontWeight', 'bold', 'Color', 'k');
% end
...
% Create a custom colormap for the convex hulls
hullMap = [0 0 0]; % Black
colormap(hullMap);
for i = unique(Y)
    scatter3(mappedX(Y == i, 1), mappedX(Y == i, 2), mappedX(Y == i, 3), 50, 'filled', 'DisplayName', ['Label ', num2str(i)], 'MarkerFaceColor', scatterMap(i,:), 'MarkerFaceAlpha', 0.8);
    hold on
end
xlabel('t-SNE Dimension 1');
ylabel('t-SNE Dimension 2');
...
% Plot cluster boundaries as convex hulls
for i = 1:numClusters
    hullPoints = mappedX(idx == i,:);
    if size(hullPoints, 1) >= 4 % Check if there are at least 4 points in the cluster
        hullIndices = convhulln(hullPoints); % Compute the convex hull
        trimesh(hullIndices, hullPoints(:,1), hullPoints(:,2), hullPoints(:,3), 'FaceAlpha', 0.1, 'EdgeColor', 'k', 'HandleVisibility', 'off', 'FaceColor', hullMap(1,:)); % Plot the convex hull as a mesh with a separate colormap
    end
end
...
% CAPS = data.CAP(combinedIndex);
% counter = 0.1;
% for cap=1:1:length(CAPS)
%     if CAPS(cap) == 0
%         CAPS(cap) = CAPS(cap) + counter;
%         counter = CAPS(cap) + 0.01;
%     end
% end
% CAPS = unique(CAPS);
% risk = (CAPS>15) & (CAPS<90); %risk
% mild = (CAPS>90) & (CAPS<115); %mild
% severe = (CAPS>115) & (CAPS<999); %severe
% riskMQ = (CAPS>0) & (CAPS<9); %mq risk
% mildMQ = (CAPS>0) & (CAPS<11.5); %mq mild
% ssevereMQ = (CAPS>11.5) & (CAPS<20); %mq severe
...
for ki=1:1:k
    ...
    disp(strcat('Premanifest - HD members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Premanifest - HD")))));
    disp(strcat('Mild - HD members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Mild - HD")))));
    disp(strcat('Severe - HD members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Severe - HD")))));
    disp(strcat('Premanifest - MitoQ members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Premanifest - MitoQ")))));
    disp(strcat('Mild - MitoQ members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Mild - MitoQ")))));
    disp(strcat('Severe - MitoQ members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Severe - MitoQ")))));
    disp(strcat('Premanifest - HDMedia members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Premanifest - HDMedia")))));
    disp(strcat('Mild - HDMedia members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Mild - HDMedia")))));
    disp(strcat('Severe - HDMedia members in cluster ', {' '}, string(ki), {' '}, 'is ', {' '}, string(sum(ismember(uniquePatientsWell(idx == ki), "Severe - HDMedia")))));
...
newcolors = {'#4285F4','#0F9D58','#F4B400','#DB4437' '#330072', '#016773'};
newcolorsRGB = {[66 133 244]/255,[15 157 88]/255,[244 180 0]/255,[219 68 55]/255, [51 0 114]/255};
folderCurrent = "C:\Cell_Migration_MATLAB\NatureAviv\Compunds2\Features\MitoQ\Plots\";
mkdir(folderCurrent);
...
for myGroup=starts:ends
        ...
        elseif strfind(currentGroupName, 'HD') > 0
            ...
            myTypeOuter = 'HD';
        elseif strfind(currentGroupName, 'MitoQ') > 0
...
temp_cellType((CAPS>20) & (CAPS<90)) = 'HD - Premanifest'; %risk
temp_cellType((CAPS>90) & (CAPS<115)) = 'HD - Mild'; %mild
temp_cellType((CAPS>115) & (CAPS<999)) = 'HD - Severe'; %severe
temp_cellType((CAPS>0) & (CAPS<9)) = 'MitoQ - Premanifest'; %mq risk
temp_cellType((CAPS>9) & (CAPS<11.5)) = 'MitoQ - Mild'; %mq mild
temp_cellType((CAPS>11.5) & (CAPS<20)) = 'MitoQ - Severe'; %mq severe

numOfRandoms = 10;
sz = 10;
   clusterNames = string(unique(temp_cellType));
    for iBig=1:1:length(clusterNames)
    starts = iBig;
    ends = iBig+0;
    if ends > length(clusterNames)
...
totalNum = 0;
   clusterNames = string(unique(temp_cellType));
   new_order = [1, 5, 6, 7, 2, 3, 4];
    clusterNames = clusterNames(new_order);
